...
description: How to authenticate with the Shanone API
---

# Authentication

Learn how to authenticate your requests to the Shanone API.
...
<Steps>
  <Step title="Sign in to Shanone">
    Go to [app.Shanone.ai](https://app.Shanone.ai) and sign in to your account.
  </Step>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.Shanone.ai/v1/sessions" \
      -H "Authorization: Bearer YOUR_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"prompt": "Hello, Shanone!"}'
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    response = requests.post(
        "https://api.Shanone.ai/v1/sessions",
        headers=headers,
        json={"prompt": "Hello, Shanone!"}
    )
    ```
  </Tab>
  <Tab title="JavaScript">
    ```javascript
    const response = await fetch("https://api.Shanone.ai/v1/sessions", {
      method: "POST",
      headers: {
        "Authorization": "Bearer YOUR_API_KEY",
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ prompt: "Hello, Shanone!" })
    });
    ```
  </Tab>
</Tabs>
