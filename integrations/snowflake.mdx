---
title: "Snowflake"
description: "Query and manage your Snowflake data warehouse — databases, tables, warehouses, tasks, and Cortex AI"
icon: /images/snowflake.svg
---

# Snowflake

Shanone's Snowflake integration gives your agent 142 tools spanning SQL execution, the full object model (databases, schemas, tables, views, warehouses), data engineering primitives (streams, tasks, pipes, dynamic/iceberg tables), governance (roles, grants, network policies), and Cortex AI — enough to run most data-platform operations directly from an agent.

## Getting Started

<Steps>
  <Step title="Generate a key pair">
    Snowflake authenticates via key-pair auth rather than a simple token. Generate an RSA key pair and assign the public key to your Snowflake user (`ALTER USER ... SET RSA_PUBLIC_KEY = '...'`).
  </Step>
  <Step title="Connect the credentials in Shanone">
    Ask your agent to do something with Snowflake (e.g. "list all warehouses and their current size"), or add your Account Identifier, Username, and Private Key (PEM) proactively from **Integrations** in the Shanone dashboard. All three fields are required.
  </Step>
  <Step title="Retry your request">
    Once connected, `shanone_execute_tool` calls for `snowflake_*` tools will succeed.
  </Step>
</Steps>

## Available Tools

Shanone provides **142 tools** for Snowflake, organized into these categories:

<AccordionGroup>
  <Accordion title="SQL execution & statements">
  ...
  <Accordion title="Databases, schemas & roles">
  ...
  <Accordion title="Warehouses & compute pools">
  ...
  <Accordion title="Data engineering (streams, tasks, pipes)">
  ...
  <Accordion title="Functions, procedures & stages">
  ...
  <Accordion title="Governance & security">
  ...
  <Accordion title="Image repositories & external volumes">
...
<CardGroup cols={2}>
  <Card title="Ad-hoc analysis" icon="database">
    Run a SQL query against a warehouse and summarize the result set in natural language
  </Card>
  <Card title="Cost management" icon="gauge">
    List warehouses, check which are running, and suspend idle ones to cut compute spend
  </Card>
  <Card title="Access review" icon="shield-check">
    List grants and roles for a database to audit who has write access before an incident review
  </Card>
  <Card title="Pipeline monitoring" icon="workflow">
    Check task and stream status across a schema to spot a broken ELT pipeline
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="Every call fails with an authentication error">
    Snowflake requires the Account Identifier, Username, and Private Key together — a partial credential set will not authenticate. Double-check the private key is the unencrypted PEM matching the public key registered on the Snowflake user.
  </Accordion>
  <Accordion title="snowflake_execute_sql returns a statement handle instead of results">
    Long-running queries execute asynchronously — poll `snowflake_get_statement_status` and `snowflake_get_statement_result` with the returned statement handle, or use `snowflake_poll_until_complete` to wait for it automatically.
  </Accordion>
  <Accordion title="A warehouse or compute pool operation fails with an insufficient privileges error">
    Most create/delete/grant operations require the connected user's role to have the relevant object privilege — use `snowflake_list_grants` to check what the current role can actually do before assuming a bug.
  </Accordion>
  <Accordion title="snowflake_create_dynamic_table or snowflake_create_stream succeeds but data isn't updating">
    Dynamic tables and streams only refresh on their configured lag/schedule or when explicitly triggered — use `snowflake_refresh_dynamic_table` or check the object's `TARGET_LAG` setting.
  </Accordion>
</AccordionGroup>
